...
#include "cartoon.h"
#include "ImageUtils.h" // Handy functions for debugging OpenCV images.
                        ...
                        bool mirrorMode)
{
    ...
    if (flippedMode || mirrorMode){
        int mirrorInt = 0;
        // Flips or Mirror the image
        if (mirrorMode)
            mirrorInt = 1;
        flip(smallImg,smallImg,mirrorInt); // OpenCV function that flips image, 0 in X axis, 1 in y axis
        flip(mask,mask,mirrorInt);
    }
